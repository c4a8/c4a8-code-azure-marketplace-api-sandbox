...
public class TenantSeedService(MarketplaceDbContext db, ILogger<TenantSeedService> logger)
{
    public async Task SeedAsync(Tenant tenant)
    {
        var publisherId = tenant.PublisherId;
        logger.LogInformation("Seeding default offers for tenant {TenantId} (publisher {PublisherId})...",
            tenant.Id, publisherId);
